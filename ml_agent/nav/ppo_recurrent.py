...
ENTROPY_COEF = 0.005   # 2026-09-22: starting value for the proportional controller (28.12).
                       # (superseded) RAISED 0.002 -> 0.01 on 2026-09-20 08:15, deliberately, to break a points
...
ENT_CUT_AT = 0.30      # the controller starts cutting HERE. Two reasons for the value.
                       # (1) LAG: cutting at the band top of 0.8 let entropy reach 0.90 and 0.91 on
                       # two measured excursions, because the coefficient moves log_std slowly and
                       # the excursion was committed before the cut began. Always leave a ~0.10 gap
                       # below the level you actually want.
                       # (2) SPEED, 2026-09-20 11:55: entropy IS steering noise. Opening the band
...
ENT_STEP_REL = 0.05    # step is max(ENT_STEP_ABS, |coef| * ENT_STEP_REL): multiplicative where the
ENT_STEP_ABS = 0.0015  # coefficient is large, additive near zero so it can CROSS zero. A purely
                       # multiplicative step cannot, which is why the controller had no way to push
...
ENT_COEF_MIN = -0.03   # NEGATIVE floor: below zero the term is an entropy PENALTY, a real
                       # downward gradient. Much smaller in magnitude than the ceiling on
                       # purpose, because driving the policy toward determinism is the
                       # failure this whole mechanism exists to prevent.
ENT_COEF_MAX = 0.25
ENT_TARGET = 0.30      # differential entropy to hold (direction std ~0.22 rad, 12-13 deg): the level the
...
_ent_coef = min(ENT_COEF_MAX_P, max(0.0, ENTROPY_COEF))      # live value, adapted by _adapt_entropy()
