...
def _adapt_entropy(ent):
    ...
    step = max(ENT_STEP_ABS, abs(_ent_coef) * ENT_STEP_REL)
    if _ent_ema < ENT_BAND_LO:
        _ent_coef = min(ENT_COEF_MAX, _ent_coef + step)
    elif _ent_ema > ENT_CUT_AT:
        _ent_coef = max(ENT_COEF_MIN, _ent_coef - step)
