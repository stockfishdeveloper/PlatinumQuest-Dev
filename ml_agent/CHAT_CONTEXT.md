...
## Observation Space (161 dims, WORLD frame since 2026-09-14)

**Edge rays added 2026-09-15 (38 dims, obs 123 -> 161).** `terrain_obs.TerrainMap.observe(raw)` now returns the 64 point samples plus 16 heading rays [edge_dist/20, what-lies-beyond dz] and 3 gem rays [clear fraction of the line to the gem, dz beyond]. A ray follows the floor it is on (ramps included) until it ends. Why: the 8x4 point samples cannot express "the floor ends 2 u ahead on the line to that gem" for a 4-unit hole, so PPO plateaued at ~80 pts / 7 OOB and human demos could not teach detours (the clone agreed with the human's detours no better than with the straight line; the BC aux term dragged the policy into holes, 47 pts / 16 OOB). Checkpoints are widened with zero columns at load (behaviour identical at the switch); recordings made before are upgraded in place by demo_data (edge rays recomputed from obs_raw). The summary line prints `EdgeCols:` next to `TerrainCols:` so you can watch the networks pick the new inputs up. Cost: 0.6 ms per observe().
...
**Terrain workflow:** `python generate_terrain_map.py <MapName>` once per map (writes the .npz and a check image), set `DEFAULT_TERRAIN_MAP` in train_ppo.py to that name, then plain `python train_ppo.py` (`--terrain` / `--no-terrain` exist only for one-off runs). Checkpoints saved before the terrain observation (59-dim) load through `widen_state_dict`: the four first layers get zero-initialized columns for the new inputs, so the loaded policy behaves identically at the switch and Adam moments are carried over. Probe scripts (`generate_policy_diagnostic.py`, `generate_brake_heatmap_map.py`) append the terrain dims automatically via `gbh.TERRAIN`.
...
- `clip_epsilon = 0.1`, `target_kl = 0.2` (early stop fires at max_kl >= 0.3; was 1.0/1.5 until 2026-09-13, which let KL-1.29 updates through)
- `entropy_coef = -0.001` (direction + throttle only; jump and brake excluded). Direction log_std clamp `[-2.0, cap]` where the cap anneals from -0.5 (35 deg) by 0.02 per update to `LOG_STD_MAX_TARGET = -1.0` (21 deg) as of 2026-09-13; the raw log_std sits above the cap so the cap is the exploration std; current cap is saved in the checkpoint; `--log-std-max` overrides the target. Throttle log_std clamp `[-3.0, -0.9]`.
- `TERRAIN_LR_MULT = 4.0` on the 38 edge-ray columns only (since 2026-09-16; the split is `obs_dim - EDGE_DIM`). The 2026-09-13 4x run on the point-sample columns hurt, but those inputs were garbage in 3 of 4 games (spawn-yaw frame bug). Revert to 1.0 if KL-STOPs pile up or points sit under 75 for 50 games.
- **No flags needed.** The training command is plain `python train_ppo.py`; every current setting is a default in the code (`DEFAULT_TERRAIN_MAP` at the top of train_ppo.py selects the terrain map). Flags are for one-off experiments only; when a setting becomes the new normal, change the default rather than adding to the command line.
...
- **Game crash after ~10 h of training (2026-09-13)**: marbleblast.exe died with an access violation at the same fault offset three nights running. observer.cs called `$MP::MyMarble.isMegaMarble()` twice per tick for fields that are never serialized, and the engine printed `Object (id) Marble -> ShapeBase -> ...` for each call: 10.2 million lines per session, a 1 GB console.log, and (with the console buffered in memory) a steady leak. Also `ClientGroup.getObject(<client id>)` in the opponent scan (index expected) fired 12,784 `Set::getObject index out of range` per session. Fixed: unused powerup/mega/opponent calls removed (placeholders kept), lookups corrected for later. train_ppo.py now survives game disconnects and waits for a reconnect instead of exiting; `ml_agent/run_game_loop.ps1` relaunches the game with `-mission KingOfTheMarble_Hunt` on exit.
