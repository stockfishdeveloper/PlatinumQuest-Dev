...
def main():
    ...
    if WATCH:
        ...
        print('WATCH mode: rendering every frame, paced to 64 ms per decision (true real time)')
    ...
    for r in range(ROUNDS):
        ...
        while True:
            if WATCH:                      # pace to wall clock: one decision per 64 ms of sim
                next_tick += 0.064
            ...
            goal, pathed = path_waypoint(terrain, float(env.msg.obs[0]), float(env.msg.obs[1]), field, gem_goal)
            if pathed:
                n_pathed += 1
            ngoal = snap_to_walkable(terrain, nxt[0], nxt[1], nxt[2])[0] if nxt else None
