> **ARCHIVED 2026-09-18.** This describes the *edge-shaping* approach on the old per-map
> `train_ppo.py` agent, superseded by the navigator (`ml_agent/nav/`, see
> `../../HANDOFF_NAV_TRAINING.md`). The navigator does not use pre-computed edge points: it
> loads the map's height stack from `terrain_maps/terrain_<map>.npz` and ray-marches the edge
> features live (`terrain_obs.py`, `nav/obs.py`), which is why it works on maps it has never
> seen. The `ml_agent/edge_maps/` data files this document refers to were DELETED on 2026-09-18;
> regenerate them with `python generate_edge_map.py <MapName>` if this approach is ever revived.
